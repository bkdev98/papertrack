...
export function Masthead() {
  ...
  return (
    <header className="px-6 pt-5 max-sm:px-4 sm:px-12">
      <div className="flex flex-wrap items-center gap-4">
        <Link to={PATHS.overview} className="flex items-baseline gap-2.5 max-sm:py-1.5">
        ...
        {/* Date + actions wrap as one right-aligned unit: gap-4 between them matches
            the parent row's gap, so single-row rendering is identical to keeping them
            as separate flex children. */}
        <div className="ml-auto flex items-center gap-4">
          <span className="hidden text-[13px] italic text-muted md:inline">{formatDateLong()}</span>
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="shadow-stamp cursor-pointer bg-ink px-4 py-[9px] font-mono text-[10.5px] uppercase tracking-[1px] text-paper transition-all duration-[180ms] max-sm:inline-flex max-sm:min-h-[44px] max-sm:items-center max-sm:justify-center hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[5px_5px_0_rgba(163,56,43,0.85)] active:scale-[0.97]"
            >
              + Thêm bài báo
            </button>
          </div>
